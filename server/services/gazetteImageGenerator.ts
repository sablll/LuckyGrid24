...
const STATE_GAZETTE_THEMES: Record<string, StateGazetteTheme> = {
  KL: {
...
export function generateOfficialGazetteSvg(result: LotteryResult): string {
  const theme = STATE_GAZETTE_THEMES[result.stateCode.toUpperCase()] || {
    ...
    darkColor: '#0f172a',
    lightBg: '#f8fafc',
    directorate: `DIRECTORATE OF ${result.stateName.toUpperCase()} STATE LOTTERIES`,
    headquarters: `Government of ${result.stateName}`
  };

  const isBumper = result.lotteryName.toUpperCase().includes('BUMPER') || (result.firstPrize.amountNumeric && result.firstPrize.amountNumeric >= 20000000);
  ...
  const width = 1000;
  const height = Math.max(1200, calculatedHeight);

  // Separate non-first prizes
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${width} ${totalSvgHeight}" width="${width}" height="${totalSvgHeight}">
  ...
  <g transform="translate(45, 38)">
    ...
    <!-- Title Text -->
    <text x="475" y="32" font-family="'Helvetica Neue', Arial, sans-serif" font-size="14" font-weight="800" fill="#fef08a" text-anchor="middle" letter-spacing="3">GOVERNMENT OF ${result.stateName.toUpperCase()}</text>
  ...
  <g transform="translate(45, 145)">
    ...
    <text x="24" y="54" font-family="'Courier New', Courier, monospace" font-size="12" font-weight="700" fill="#475569">
      DRAW NO: <tspan font-weight="900" fill="${theme.primaryColor}">${escapeXml(result.drawNumber)}</tspan> | GAZETTE REF: ${escapeXml(result.officialSource?.gazetteNotificationNo || `KL-LOT-${result.drawNumber}`)}
    </text>
